...
data = read.csv('GERAL_Workshop_v6_VITOR.csv', sep = ",", header = FALSE)
...
subdata = (subset(mydata, 
                  (Experimento == 'Resist̻ncia' & (SubExp == 'Seca' | SubExp == 'Verde' | SubExp == 'Queimada')
                   & TS == '1 mm')))
...
bwplot(Amostra ~ Valor | SubExp + AB, data = subdata,
       scales = list(x = 'free'))
...
g = ggplot(subdata, aes(x = Amostra, y = Valor)) + 
  ...
  xlab("") + 
  ylab("Resist̻ncia (kgf/pelota)") + 
  ggtitle(paste("Resist̻ncia das pelotas","secas","de granulometria",A)) + 
