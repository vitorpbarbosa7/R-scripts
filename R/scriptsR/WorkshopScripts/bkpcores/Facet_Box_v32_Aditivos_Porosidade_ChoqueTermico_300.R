...
subdata = (subset(mydata, Experimento == 'Porosidade' &
                   SubExp == 'Choque Térmico' & AB == "B" & SubSubExp == "300 °C" &
                   TS == "1 mm")
                   )
...
addlevels = c('0.50% Bentonita  ','0.50% Bentonita 0.04% TPP','0.50% Bentonita 0.04% Arkomon',
              ...
              '0.04% Arkomon  ','0.15% Arkomon  ','0.04% PEO  ','0.04% Amido  ')
...
cores = c('#0289ba',  '#00B0F0',  '#33E0FF',  '#C733FF',  '#841dab',  '#25FF2F',  '#14c91c',  '#FF0000',  '#b50202',  '#f257e9',  '#FFC204'
)
...
tiff('imgs/facet/Workshop/Porosidade_ChoqueTermico_300.tiff', width = dim$w, height = dim$h, res = dim$res)
g
dev.off()
