...
g = ggplot(subdata, aes(x = Amostra, y = Valor)) + 
  ...
  xlab("") + 
  ylab("Porosidade (%)") + 
  ggtitle(paste("Porosidade das pelotas","secas","de granulometria",A)) + 
...
tiff('imgs/facet/Workshop/Porosidade_Seca_A.tiff', width = dim$w, height = dim$h, res = dim$res)
g
dev.off()
