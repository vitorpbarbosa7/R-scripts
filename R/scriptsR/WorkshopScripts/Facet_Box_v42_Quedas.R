...
subdata = subset(mydata, 
                 (Experimento == 'N° de Quedas' & TS == '1 mm') |
                  (Experimento == "N° de Quedas" & TS == 'PFM' & Aditivo1 == 'Bentonita Nacional'))
...
g = ggplot(subdata, aes(x = Amostra, y = Valor)) + 
  geom_boxplot(aes(colour = Amostra, fill = Amostra), alpha = 0.3, outlier.size = 0.8) + 
...
tiff('imgs/facet/Workshop/Quedas_Workshop.tiff', width = dim$w, height = dim$h, res = dim$res)
g
dev.off()
