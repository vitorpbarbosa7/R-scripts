...
subdata = (subset(mydata, 
                  (Experimento == 'Resistência' & SubExp == 'Seca' & AB == "B" & TS == "1 mm") |
                    (Experimento == 'Resistência' &
                       SubExp == 'Seca' & AB == "B" & TS == "PFM" & Aditivo1 == "Bentonita Nacional")))
...
addlevels = c('0.50% Bentonita Nacional  ', '0.04% Peridur 0.02% NaOH','0.12% Peridur 0.02% NaOH',
              '0.04% Arkomon  ','0.15% Arkomon  ','0.04% TPP  ','0.15% TPP  ',
              '0.50% Bentonita Nacional 0.04% Arkomon','0.50% Bentonita Nacional 0.04% TPP',
              '0.04% PEO  ','0.12% PEO  ','0.04% Amido  ')
...
g = ggplot(subdata, aes(x = Amostra, y = Valor)) + 
  ...
  ggtitle(paste("Resistência das pelotas","secas","de granulometria",B)) + 
...
tiff('imgs/facet/Workshop/Resistencia_Seca_B.tiff', width = dim$w, height = dim$h, res = dim$res)
g
dev.off()
