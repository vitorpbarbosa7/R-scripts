...
subdata = (subset(mydata, 
                  (Experimento == 'Resistência' & SubExp == 'Verde' & AB == "B" & TS == "1 mm") |
                    (Experimento == 'Resistência' &
                       SubExp == 'Verde' & AB == "B" & TS == "PFM" & Aditivo1 == "Bentonita Nacional")))
...
tiff('imgs/facet/Workshop/Resistencia_Verde_B.tiff', width = dim$w, height = dim$h, res = dim$res)
g
dev.off()
