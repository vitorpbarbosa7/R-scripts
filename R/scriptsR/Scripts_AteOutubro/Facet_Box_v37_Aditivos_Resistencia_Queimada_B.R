...
subdata = (subset(mydata, 
                  (Experimento == 'Resistência' &
                     SubExp == 'Queimada' & TS == "1 mm" & AB == 'B') |
                    (Experimento == 'Resistência' &
                       SubExp == 'Queimada' & 
                       TS == 'PFM' & Aditivo1 == "Bentonita Nacional" & AB == 'B')
)
)
...
tiff('imgs/facet/Workshop/Resistencia_Queimada_B.tiff', width = dim$w, height = dim$h, res = dim$res)
g
dev.off()
