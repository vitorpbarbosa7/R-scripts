...
mydata$Amostra = as.factor(paste(mydata$Minerio,mydata$Dose1,mydata$Aditivo1,
...
subdata = (subset(mydata, 
                  (Ensaio == 'E33' & (SubExp == 'Verde' | SubExp == 'Curada UV'))
)
)
...
medias = subdata %>%
  group_by(Ensaio, SubExp, AB, Amostra) %>%
...
g = ggplot(subdata, aes(x = Amostra, y = Valor)) + 
  ...
  ggtitle(paste("Resistência das pelotas confeccionadas com resina")) + 
...
tiff('imgs/facet/Workshop/Resistencia_Cura_UV.tiff', width = dim$w, height = dim$h, res = dim$res)
g
dev.off()
