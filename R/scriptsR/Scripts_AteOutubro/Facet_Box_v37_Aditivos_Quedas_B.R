...
subdata = (subset(mydata, 
                  (Experimento == 'N° de Quedas' & TS == "1 mm" & AB == 'B') |
                    (Experimento == 'N° de Quedas' & TS == 'PFM' & AB == 'B' & Aditivo1 == "Bentonita Nacional")
)
)
...
tiff('imgs/facet/Workshop/Resistencia_Quedas_B.tiff', width = dim$w, height = dim$h, res = dim$res)
g
dev.off()
