...
mydata = melt(data = data2,
              id.vars = names(data2[c(1:13)]),
              measure.vars = names(data2[-c(1:13)])
              )
names(mydata)[c(14:15)] = c("Replicata","Valor")
names(mydata)[6] = "AB"
...
# subdata$Amostra = sub("PFM PFM 0.50% Bentonita   A","PFM 0.50% Bentonita   A",subdata$Amostra)
# subdata$Amostra = sub("PFM PFM 0.50% Bentonita   B","PFM 0.50% Bentonita   B",subdata$Amostra)
# subdata$Amostra = as.factor(subdata$Amostra)

subdata$AB = fct_rev(subdata$AB)

g = ggplot(subdata, aes(x = Amostra, y = Valor)) + 
  geom_boxplot(aes(colour = Aditivo, fill = Aditivo), alpha = 0.3, outlier.size = 2) + 
  facet_grid(.~AB, scales = "free") + 
  geom_text(data = medias, aes(label = media, y = media + 0.01),size =5) +
  xlab("") + 
  ylab("Resistência (kgf/pelota)") + 
  ggtitle(paste("Resistência das pelotas queimadas")) + 
...
tiff('imgs/facet/Workshop/Queimada.tiff', width = dim$w, height = dim$h, res = dim$res)
g
dev.off()
