...
subdata = (subset(mydata, Experimento == 'Porosidade' &
                   SubExp == 'Verde' & AB == "A" & (TS == "1 mm" | TS == "4,74 mm" | TS == "PFM")))
...
g = ggplot(subdata, aes(x = Amostra, y = Valor)) + 
  ...
  xlab("") + 
  ylab("Porosidade (%)") + 
  ggtitle(paste("Porosidade das pelotas","verdes","de granulometria",A)) + 
...
tiff('imgs/facet/Parcial_3/Porosidade_Verde_A.tiff', width = dim$w, height = dim$h, res = dim$res)
g
dev.off()
