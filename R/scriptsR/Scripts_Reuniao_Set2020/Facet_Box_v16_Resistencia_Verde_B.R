...
subdata = (subset(mydata, Experimento == 'Resistência' &
                   SubExp == 'Verde' & AB == "B" & (TS == "1 mm" | TS == "4,74 mm" | TS == "PFM")))
...
g = ggplot(subdata, aes(x = Amostra, y = Valor)) + 
  ...
  geom_text(data = medias, aes(label = media, y = media + 0.001),size =5) +
  xlab("") + 
  ylab("Resistência (kgf/pelota)") + 
  ggtitle(paste("Resistências das pelotas","verdes","de granulometria",B)) + 
...
tiff('imgs/facet/Parcial_3/Resistencia_Verde_B.tiff', width = dim$w, height = dim$h, res = dim$res)
g
dev.off()
