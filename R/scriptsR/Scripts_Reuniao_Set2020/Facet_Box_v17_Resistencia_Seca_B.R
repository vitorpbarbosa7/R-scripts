...
subdata = (subset(mydata, Experimento == 'Resistência' &
                   SubExp == 'Seca' & AB == "B" & (TS == "1 mm" | TS == "4,74 mm" | TS == "PFM")))
...
g = ggplot(subdata, aes(x = Amostra, y = Valor)) + 
  ...
  ggtitle(paste("Resistências das pelotas","secas","de granulometria",B)) + 
...
tiff('imgs/facet/Parcial_3/Resistencia_Seca_B.tiff', width = dim$w, height = dim$h, res = dim$res)
g
dev.off()
