...
# Plot --------------------------------------------------------------------
subdata = subset(mydata, (Experimento == 'Porosidade' | Experimento == 'Resistência') &
                   SubExp == 'Queimada' &
                   Minério == 'PFM')
...
g = ggplot(subdata, aes(x = A.B, y = Valor)) + 
  ...
  scale_y_continuous(name = "Resistências (kgf/pelota)", 
                     sec.axis = sec.axis(name = "Porosidade (-)")) + 
...
tiff('imgs/facet/Resis_PelletFeed_Queimada.tiff', width = 1200, height = 1000, res = 120)
g
dev.off()
