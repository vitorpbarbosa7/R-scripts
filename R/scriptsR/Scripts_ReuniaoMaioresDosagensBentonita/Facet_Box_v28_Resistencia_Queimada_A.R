...
subdata = (subset(mydata, 
                  (Experimento == 'Resistência' & SubExp == 'Queimada' & AB == "A" & TS == "1 mm") |
                    (Experimento == 'Resistência' &
                       SubExp == 'Queimada' & AB == "A" & TS == "PFM" & Aditivo1 == "Bentonita Nacional")))
...
g = ggplot(subdata, aes(x = Amostra, y = Valor)) + 
  ...
  ggtitle(paste("Resistência das pelotas","Queimadas","de granulometria",A)) + 
...
tiff('imgs/facet/Workshop/Resistencia_Queimada_A.tiff', width = dim$w, height = dim$h, res = dim$res,
     units = 'in')
g
dev.off()
