...
subdata = (subset(mydata, 
                  (Experimento == 'N° de Quedas' & AB == "B" & TS == "1 mm") |
                    (Experimento == 'N° de Quedas' & AB == "B" & TS == "PFM" & Aditivo1 == "Bentonita Nacional")))
...
g = ggplot(subdata, aes(x = Amostra, y = Valor)) + 
  ...
  xlab("") + 
  ylab("N° de quedas (-)") + 
  ggtitle(paste("N° de quedas das pelotas de granulometria",B)) + 
...
tiff('imgs/facet/Workshop/Quedas_B.tiff', width = dim$w, height = dim$h, res = dim$res,
     units = 'in')
g
dev.off()
