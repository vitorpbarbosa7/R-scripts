string = c('Pellet Feed abla bla','CG abla bla')

string2 = gsub("Pellet Feed","PFM",string)
...
unit = '(-)'
  ...
  g = ggplot(meltdata, aes(x = Amostra, y = meltdata[,2])) + 
    ...
    geom_text(data = medias, aes(label = media, y = media + 0.01)) + #Possível deslocar fazendo media + 0.01 por ex.
