...
g = ggplot(meltdata, aes(x = Amostra, y = meltdata[,2])) + 
  ...
  stat_summary(fun.data = fun_mean,3, geom="text", vjust=-0.7) + 
  ylab(ylab) + 
  xlab(' ') + 
  ggtitle(titulo)
...
ggbox('csvs\\ResisChoqTer300.csv', 'Resistência Choque Térmico','Resistência Choque Térmico 300 °C','Resistência Choque Térmico 300 °C')
dev.off()

# Resistência Choque Térmico 500 °C -------------------------------------
tiff(paste(img,'Resistência Choque Térmico 500 °C','.tiff',sep=""), width = w, height = h, res = res)
ggbox('csvs\\ResisChoqTer500.csv', 'Resistência Choque Térmico','Resistência Choque Térmico 500 °C','Resistência Choque Térmico 500 °C')
dev.off()
