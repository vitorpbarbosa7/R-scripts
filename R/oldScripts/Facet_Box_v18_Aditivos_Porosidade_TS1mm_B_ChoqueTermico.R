...
subdata = (subset(mydata, Experimento == 'Porosidade' &
                   SubExp == 'Choque Térmico' & AB == "B" & TS == "1 mm"))
...
g = ggplot(subdata, aes(x = Amostra, y = Valor)) + 
  ...
  ggtitle(paste("Minério: 1 mm;", "Granulometria: -12,5 mm +9,5 mm")) + 
  scale_color_manual(values = cores, breaks = levels(factor(subdata$Aditivo))) + 
  theme  
g


tiff('imgs/facet/Parcial_3/Aditivos_Porosidade_TS1mm_B_ChoqueTermico.tiff', width = 1600, height = 1000, res = 120)
g
dev.off()
