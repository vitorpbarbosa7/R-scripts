...
g = ggplot(subdata, aes(x = Amostra, y = Valor)) + 
  geom_boxplot(aes(color = SubSubExp, fill = Aditivo),
               outlier.size = 0.8) + 
  facet_wrap(.~TS, scales = "free") + 
  # geom_text(data = medias, aes(label = media, y = media + 0.01),size =5) +
  xlab("") + 
  ylab(paste("Porosidade","(%)")) + 
  scale_fill_manual(values = cores, breaks = levels(factor(subdata$Aditivo))) + 
  theme  
g


tiff('imgs/facet/Parcial_3/SUBSUBEXP_TS.tiff', width = 1600, height = 1000, res = 120)
g
dev.off()
