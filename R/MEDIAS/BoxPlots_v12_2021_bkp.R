...
mydata = melt(data = data2,
              id.vars = names(data2[c(1:21)]),
              measure.vars = names(data2[-c(1:21)])
)

names(mydata)[c(22:23)] = c("Replicata","Valor")
names(mydata)[8] = "AB"
names(mydata)[1] = 'Experimento'

# Converter todas colunas para fatores
mydata[names(mydata[c(1:22)])] = lapply(mydata[names(mydata[c(1:20)])], factor)
...
# Verificar quais Experimentos estão dispon??veis
...
# Vamos precisar desse agora para distinguir a pelotaiza??o em camadas
...
mydata_new = 
  ...
  mutate(
    Familia = case_when(
      ...
      str_detect(Aditivo2, "Peridur") ~ "Peridur", 
      ...
      str_detect(Aditivo1, "Bentonita Nacional") ~ "Refer?ncias",
      TRUE ~ Familia
    )
  )
...
subdesgraca = subdata
 ...
 # Deixar Bentonita Nacional para depois nos nomes, na ra?a
...
# Deixar na ordem desejada  
 subdata$Amostra = ordered(subdata$Amostra, 
                           levels = c("PFM  0.50% Bentonita Nacional",
                                      ...
                                      "PFNM 1 mm 10.00% PFM 0.06% Peridur 0.02% NaOH 0.70% Bentonita Nacional",
                                      "PFNM 1 mm 5.00% Lama 0.70% Bentonita Nacional",
                                      "PFNM 1 mm 5.00% Lama 0.02% NaOH 1.50% Bentonita Nacional",
                                      "PFNM 1 mm 5.00% Lama 0.06% Peridur 0.02% NaOH 0.70% Bentonita Nacional",
                                      "PFNM 1 mm 5.00% Lama 0.06% Peridur 0.02% NaOH 0.50% Bentonita Nacional",
                                      "PFNM 1 mm 5.00% Lama 0.06% Peridur 0.02% NaOH 0.35% Bentonita Nacional",
                                      "PFNM 1 mm 10.00% Lama 0.06% Peridur 0.02% NaOH 0.70% Bentonita Nacional",
                                      "PFNM 1 mm 10.00% Lama 0.06% Peridur 0.02% NaOH 0.70% Bentonita Nacional Camadas",
...
subdata$Familia = ordered(subdata$Familia, 
                          levels = c('Lama','PFM','Peridur','Amido ou PEO','Arkomon ou TPP','Refer?ncias'))
levels(subdata$Familia)
table(subdata$Familia)
str(subdata)
cores = c('Lama' = 	'#b7410e','Amido ou PEO' = '#3540f3', 'Arkomon ou TPP' = '#f2f637', 'Refer?ncias' = '#f34f35',
          'Peridur' = '#35f33e', 'PFM' = '#ffa500')

# limites do looping (Temos apenas 4 variáveis, resistencias verde, seca, queimada e n?mero de quedas)
init = 1
end = 4
loopingdata = subdata
