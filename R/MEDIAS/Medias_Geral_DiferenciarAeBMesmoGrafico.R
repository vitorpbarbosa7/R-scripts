...
medias_new = 
  ...
  mutate(
    Familia = case_when(
      ...
      str_detect(Aditivos, "Bentonita Nacional") ~ "AAReferências", # Esse AA está aqui para ser possível ordenar, no arrange do dplyr com eles em primeiro
      TRUE ~ Familia
    )
  )
...
medias_new = 
  ...
  mutate(
    Referencia = case_when(
      str_detect(Familia, 'AAReferências') ~ 'Sim',
      str_detect(Familia, 'Arkomon ou TPP') ~ 'Não',
      str_detect(Familia, 'Amido ou PEO')  ~ 'Não',
      str_detect(Familia, 'Peridur') ~ 'Não',
      TRUE ~ Referencia
    )
  )

# Reordenar de acordo com as famílias para facilitar renomear lá as referências para ficarem em todas famílias
medias = medias_new
medias = 
  medias %>%
  arrange(Familia, Minerio)

# Criar a sequência que será atribuída às 12 primeiras linhas
refs_family = rep(rep(c('Arkomon ou TPP','Amido ou PEO','Peridur'), each = 2),2)

#Atribuir estes nomes às 12 primeiras linhas
medias[1:12,ncol(medias)-1] = refs_family

# Reordenar as colunas
medias = medias[,c(1:10,26:28,11:25)]

# Para gravar em csv
df = medias

# Para realizar os plots
write.csv(df, file = 'outputdata/df_plot_AB_Referencias.csv', row.names = FALSE, na = '')

df_write = df

df_write[c(1:9,11:13)] = NULL
write.csv(df_write, file = 'outputdata/df_AB.csv', row.names = FALSE, na = '')
