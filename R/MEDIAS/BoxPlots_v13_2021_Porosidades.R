...
subdata = (subset(mydata, 
                  (Experimento %in% c('Porosidade') & TS == "1 mm") |
                    (Experimento %in% c('Porosidade') & Minerio == "PFM" & Aditivo1 == "BN")))
...
subdata$Familia = ordered(subdata$Familia, 
                          levels = c('Lama','PFM','Peridur','Amido ou PEO','Arkomon ou TPP','Referências'))
levels(subdata$Familia)
table(subdata$Familia)
str(subdata)
...
# limites do looping (Temos apenas 4 varià¡veis, resistencias verde, seca, queimada e Número de quedas)
init = 1
end = 3
loopingdata = subdata
...
for (i in 1:length(granulist)){
  ...
  for (j in init:end){
    ...
    subdata = (subset(loopingdata, 
                      ...
                      |
                      Exp == experimentos[j] & AB == granulist[i] & Minerio == "PFM" & Aditivo1 == "BN"))
    ...
    glist1[[index]] = ggplot(subdata, aes(x = reorder(Amostra, desc(Amostra)), 
                                                      y = Valor)) + 
      geom_boxplot(aes(color = Familia, fill = Familia), alpha = 0.4, outlier.size = 0.8) + 
      coord_flip() + 
...
path = 'C:/Users/vpb/IPT/Francisco Junior Batista Pedrosa (FIPT) - Aglomeração de grossos/!Ana/Pelotizacao/Boxplot/R/MEDIAS/outputdata/plots/Boxplots/NOVA_REFERENCIA_46/Por'
