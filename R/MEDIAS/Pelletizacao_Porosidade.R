...
subdata = subset(subdata, Experimento == 'Porosidade')
...
gpor = ggplot(subdata, aes(x = Amostra, y = Valor)) + 
  ...
  facet_rep_wrap(.~SubExp, repeat.tick.labels = TRUE) + 
...
tiff('outputdata/plots/pelletizacao/porosidade.tiff', units = 'in', width = 12, height = 6, res = 200)
gpor
dev.off()
