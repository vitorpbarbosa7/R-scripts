...
medias$Familia1 = as.factor(paste(medias$Aditivo1, medias$Aditivo2, medias$Aditivo3))
levels(medias$Familia1)
medias$Familia1 = as.character(medias$Familia1)

# Familia Bentonita e Dispersantes dispersantes

library(stringr)

str_detect(medias$Familia1, 'Arkomon')
...
medias_new = 
  medias %>%
    mutate(
      Familia1 = case_when(
      str_detect(Familia1, "Arkomon") ~ "Arkomon ou TPP",
      str_detect(Familia1, "TPP") ~ "Arkomon ou TPP",
      str_detect(Familia1, "PEO") ~ "Amido ou PEO",
      str_detect(Familia1, "Amido") ~ "Amido ou PEO",
      str_detect(Familia1, "Peridur") ~ "Peridur",
      TRUE ~ Familia1
    )
  )
