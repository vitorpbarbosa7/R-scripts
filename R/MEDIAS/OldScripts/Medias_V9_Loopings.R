...
nquedasa = subset(medias, Experimento == 'N° de Quedas' & AB == 'A')
nquedasa[del] = NULL
names(nquedasa)[c(ncol(nquedasa)-1,ncol(nquedasa))] = c('N_Quedas_A_Media','N_Quedas_A_SD')
...
glist = list()
index = 0
testlimit = length(varnamesa)
# Todos em função da Dispersão
for (j in 1:testlimit){
  for (i in 1:testlimit) {
    if (i != j){
      index = index + 1
      glist[[index]] = ggplot(df_A, aes_string(x = varnamesa[j], y = varnamesa[i])) + 
        ...
        theme + 
        scale_x_continuous(limits=c(0,max(df_A[,varnamesa[j]])*1.05)) + # Selecionar max de acordo com a coluna
        scale_y_continuous(limits=c(0,max(df_A[,varnamesa[i]])*1.05)) + 
        xlab(legenda[j,2]) + 
        ylab(legenda[i,2]) + 
...
for (j in 1:testlimit){
  for (i in 1:testlimit){
    if (i != j){
      index = index + 1
      print(index)
      mypath = file.path(path,paste(varnamesa[j],' X ',varnamesa[i]), sep = '')
      filename = paste(mypath, '.tiff', sep = '')
      tiff(filename, units = 'in', width = 12, height = 6, res = 150)
      print(glist[[index]])
      dev.off()
    }
  }
...
# Seca x Dispersao
agsecadispf1 = ggplot(data = df_A, mapping = aes(x = Dispersao_Media_A, y = Res_Seca_A_Media)) + 
  geom_point(mapping = aes(fill = Aditivo1,  colour = Aditivo1, shape = ID), size = size) + 
...
agqueidispf1 = ggplot(data = df_A, mapping = aes(x = Dispersao_Media_A, y = Res_Queimada_A_Media)) + 
  geom_point(mapping = aes(fill = Aditivo1,  colour = Aditivo1, shape = ID), size = size) + 
...
agqueidispf1
# D10_A x Dispersao
agd10dispf1 = ggplot(data = df_A, mapping = aes(x = Dispersao_Media_A, y = D10_A_Media)) + 
  geom_point(mapping = aes(fill = Aditivo1,  colour = Aditivo1, shape = ID), size = size) + 
  scale_shape_manual(values=1:nlevels(df_A$ID)) + 
  facet_wrap(.~Familia1, scales = 'free') + 
  theme + 
...
agd10dispf1
# D90_A x Dispersao
agd90dispf1 = ggplot(data = df_A, mapping = aes(x = Dispersao_Media_A, y = D90_A_Medi)) + 
  geom_point(mapping = aes(fill = Aditivo1,  colour = Aditivo1, shape = ID), size = size) + 
  scale_shape_manual(values=1:nlevels(df_A$ID)) +
  facet_wrap(.~Familia1, scales = 'free') + 
  theme + 
...
agggplot(data = df_A, mapping = aes(x = D90_A_Media, y = Res_Verde_A_Media)) + 
  geom_point(mapping = aes(fill = Aditivo1,  colour = Aditivo1, shape = ID), size = size) + 
  scale_shape_manual(values=1:nlevels(df_A$ID)) + 
  facet_wrap(.~Familia1, scales = 'free') + 
...
# Quedas x Dispersao
ggplot(data = df_A, mapping = aes(x = Dispersao_Media_A, y = N_Quedas_A_Media)) + 
  geom_point(mapping = aes(fill = Aditivo1,  colour = Aditivo1, shape = ID), size = size) + 
  geom_errorbar(aes(ymin = N_Quedas_A_Media - N_Quedas_A_SD,
                    ymax = N_Quedas_A_Media + N_Quedas_A_SD)) + 
  scale_shape_manual(values=1:nlevels(df_A$ID)) +
  facet_wrap(.~Familia1, scales = 'free') + 
  theme + 
  scale_x_continuous(limits=c(0,4)) + 
...
# Quedas x Verde
ggplot(data = df_A, mapping = aes(x = N_Quedas_A_Media, y = Res_Verde_A_Media)) + 
  geom_point(mapping = aes(fill = Aditivo1,  colour = Aditivo1, shape = ID), size = size) + 
  scale_shape_manual(values=1:nlevels(df_A$ID)) + 
  facet_wrap(.~Familia1, scales = 'free') + 
  geom_errorbar(aes(ymin = Res_Verde_A_Media - Res_Verde_A_SD,
                    ymax = Res_Verde_A_Media + Res_Verde_A_SD,
                    xmin = N_Quedas_A_Media - N_Quedas_A_SD, 
                    xmax = N_Quedas_A_Media + N_Quedas_A_SD)) + 
  theme + 
  scale_x_continuous(limits=c(0,7.5)) + 
  scale_y_continuous(limits=c(0,1.5))

# Quedas x Seca
ggplot(data = df_A, mapping = aes(x = N_Quedas_A_Media, y = Res_Seca_A_Media)) + 
  geom_point(mapping = aes(fill = Aditivo1,  colour = Aditivo1, shape = ID), size = size) + 
  scale_shape_manual(values=1:nlevels(df_A$ID)) + 
  facet_wrap(.~Familia1, scales = 'free') + 
  geom_errorbar(aes(ymin = Res_Seca_A_Media - Res_Seca_A_SD,
...
# Quedas x Queimada
ggplot(data = df_A, mapping = aes(x = N_Quedas_A_Media, y = Res_Queimada_A_Media)) + 
  geom_point(mapping = aes(fill = Aditivo1,  colour = Aditivo1, shape = ID), size = size) + 
  scale_shape_manual(values=1:nlevels(df_A$ID)) + 
  facet_wrap(.~Familia1, scales = 'free') + 
  geom_errorbar(aes(ymin = Res_Queimada_A_Media - Res_Queimada_A_SD,
