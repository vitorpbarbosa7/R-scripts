...
# disp
disp = subset(medias, Experimento == 'Dispersao' & AB == 'A') 
names(disp)[c(ncol(resverdea)-1,ncol(resverdea))] = c('Dispersao_Media','disp_sd')

# Res_verde é LEFT, porque possui maior número de registros no geral
# No final é para terem o mesmo número de experimentos, mas até 11/11/2020, não tem todos dispersão
df_merge = merge(resverdea, disp, all.x = TRUE, all.y = FALSE, all = FALSE)
...
# Multiple merge with mapreduce
df_ = Reduce(function(x, y) merge(x, y), list(resverdea, resverdeb, ressecaa, disp))
