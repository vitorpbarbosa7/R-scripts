...
medias_a = subset(medias, AB == 'A')

# Criar a variável referência de chave (os experimentos)
medias_a$Exp = paste(medias_a$Experimento,medias_a$SubExp)

# Então drop experimento
medias_a$Experimento = NULL
medias_a$SubExp = NULL
medias_a$desvpad = NULL
...
write.csv(df_A, file = 'outputdata/df_plot_2.csv', row.names = FALSE, na = '')

df_write = df_A

df_write[c(1:9,11:13)] = NULL
write.csv(df_write, file = 'outputdata/df_A_2.csv', row.names = FALSE, na = '')


# Para B: -----------------------------------------------------------------
medias_b = subset(medias, AB == 'B')

# Criar a variável referência de chave (os experimentos)
medias_b$Exp = paste(medias_b$Experimento,medias_b$SubExp)

# Então drop experimento
medias_b$Experimento = NULL
medias_b$SubExp = NULL
medias_b$desvpad = NULL
...
# Para realizar os plots
write.csv(df_B, file = 'outputdata/df_plot_B.csv', row.names = FALSE, na = '')

df_write = df_B

df_write[c(1:9,11:13)] = NULL
write.csv(df_write, file = 'outputdata/df_B_2.csv', row.names = FALSE, na = '')
