...
subdata = (subset(mydata, 
                  (Experimento %in% c('Porosidade') & TS == "1 mm") |
                    (Experimento %in% c('Porosidade') & Minerio == "PFM" & Aditivo1 == "Bentonita Nacional")))
...
# Cores 
subdata$Familia2 = droplevels(subdata$Familia2)
levels(subdata$Familia2)
cores = c('Amido ou PEO' = '#3540f3', 'Arkomon ou TPP' = '#f2f637', 'Bentonita Nacional' = '#35f33e',
           'Peridur' = '#f34f35')
...
for (i in 1:length(granulist)){
  ...
  for (j in init:end){
    ...
    glist1[[index]] = ggplot(subdata, aes(x = Amostra, y = Valor)) + 
      geom_boxplot(aes(color = Familia2, fill = Familia2), alpha = 0.5, outlier.size = 0.8) + 
      coord_flip() + 
      ...
      ggtitle(paste(legenda[j,3],'de granulometria',granulist[i])) + 
...
path = 'C:/Users/vitor/IPT/Francisco Junior Batista Pedrosa (FIPT) - AgloGrossos/!Ana/Pelotizacao/Boxplot/R/MEDIAS/outputdata/plots/Boxplots/Por'
path
