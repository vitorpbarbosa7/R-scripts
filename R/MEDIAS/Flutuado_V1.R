...
#Remover o ensaio chato que não sai
mydata = mydata[mydata$ID != 'PFNM 0.12% Peridur 0.04% NaOH   A',]
...
mydata = subset(mydata, (TS %in% c('1 mm', '')) & AB == 'A')
...
subdata$Familia = as.factor(trimws(subdata$Familia))
...
subdata$Valor = round(subdata$Valor,2)
...
theme = theme_bw(base_size = 15) + 
  theme(legend.position = 'top',
        ...
        plot.margin = margin(4, 2, 2, 2, "cm")) #Cor do background dos t?tulos de cada 

novo = c('D10','D50','D90')
names(novo) = c('D10','D50','D90')
novo

g = ggplot(loopingdata, aes(x = Amostra, y = Valor)) +
  geom_point(aes(color = Familia, fill = Familia), size = 5) +
  geom_segment(aes(x = Amostra, xend = Amostra, y = 0, yend = Valor,
                   color = Familia, fill = Familia), alpha = 0.5, size = 2) +
  coord_flip() +
  geom_text(aes(label = Valor), hjust = 1.2, vjust = -0.09, size = 6) + 
  facet_grid(.~Experimento, scales = "free", 
             labeller = labeller(Experimento = novo)) +
  xlab("") +
  ylab("(µm)") +
...
tiff('outputdata/plots/Flutuado/geral.tiff', width = 16, height = 14, units = 'in', res = 300)
g
dev.off()
...
theme2 = theme_bw(base_size = 15) + 
  theme(legend.title = element_text(size = 18),
        legend.text = element_text(size = 12),
        legend.position = c(0,-1),
        legend.direction = 'horizontal',#Posi??o da legenda
        plot.title = element_text(hjust = +.5), #Posi??o do t?tulo
        panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
        axis.text.x=element_text(colour="black", size = 16),
...
for (j in init:end){
  ...
  subdata = (subset(loopingdata, 
                    (Experimento == experimentos[j])))
  
  glist1[[index]] = ggplot(subdata, aes(x = Amostra, y = Valor)) + 
    geom_point(aes(color = Familia, fill = Familia), size = 5) + 
    geom_segment(aes(x = Amostra, xend = Amostra, y = 0, yend = Valor, 
                     color = Familia, fill = Familia), alpha = 0.5, size = 3) +
    coord_flip() + 
    geom_text(aes(label = Valor), hjust = +1.2, size = 6) + 
    xlab("") + 
    ylab('µm') + 
    scale_color_manual(name = 'Família', values =  cores) +
    scale_fill_manual(name = 'Família', values =  cores) +
...
path = 'C:/Users/vitor/IPT/Francisco Junior Batista Pedrosa (FIPT) - AgloGrossos/!Ana/Pelotizacao/Boxplot/R/MEDIAS/outputdata/plots/Flutuado'
path
index = 0; w = 12; h = 12; res = 300;
# Looping para salvar as diferentes granulometrias das pelotas
for (j in init:end){
  print(index)
  index = index + 1
  print(index)
  mypath = file.path(path,experimentos[j])
